...
import { Testimonial } from "@/interfaces/TestimonialInterfaces";
...
const TestimonialSlider: React.FC = () => {
    ...
    const renderStars = (rating: number) => {
        return Array.from({ length: 5 }, (_, index) => (
            <svg
                ...
                className={`${
                    index < rating ? "text-gray-300" : "text-[#B39852]"
                } inline-block`}
            >
                <path
                    ...
                    fill={index < rating ? "#FFFFFF" : "#D1D5DB"} // Gold for filled stars, gray for empty stars
                />
            </svg>
        ));
    };
    ...
    useEffect(() => {
        const getTestimonials = async () => {
            ...
            if(response) {
                const filteredTestimonials = response.filter((testimonial: Testimonial) => testimonial.showOnHome);
                                ...
                                <div className="flex flex-col z-10 z-10 relative">
                                    <span>
                                        <svg width="89" height="22" viewBox="0 0 89 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <path d="M85.1768 6.83228V14.7328H83.5312V6.83228H85.1768Z" fill="#FFFFFF"/>
                                            <path d="M10.926 2.51855C6.32511 2.51855 2.5957 6.24867 2.5957 10.8489C2.5957 15.4497 6.32511 19.1791 10.926 19.1791C15.5262 19.1791 19.2563 15.449 19.2563 10.8489C19.2563 6.24867 15.5262 2.51855 10.926 2.51855ZM8.88524 13.0858C8.88524 13.536 8.52041 13.9001 8.07091 13.9001C7.6207 13.9001 7.25658 13.536 7.25658 13.0858V9.61395C7.25658 9.16374 7.6207 8.79891 8.07091 8.79891C8.52041 8.79891 8.88524 9.16374 8.88524 9.61395V13.0858ZM14.6378 13.0272C14.6378 13.6637 14.2659 13.8917 13.6294 13.8917H10.7009C10.0644 13.8917 9.54856 13.3758 9.54856 12.7393V9.85881C9.54856 9.85881 9.4914 9.37967 10.0221 8.92946C10.322 8.67472 10.7672 8.18852 11.0784 7.59859C11.6966 6.42649 12.0663 6.08495 12.3479 6.17669C13.3909 6.5147 12.8694 8.0608 12.5328 8.70647H13.4854C14.1212 8.70647 14.6378 9.22231 14.6378 9.85881V13.0272Z" fill="#FFFFFF"/>
                                        </svg>
                                    </span>
                                </div>
                                    <span className="flex items-center z-10 relative">{renderStars(testimonial.rating || 5)}</span>
                                {/* Profile Picture */}
                                <div className="w-24 h-24 mx-auto rounded-full overflow-hidden mb-4 z-20 relative mt-8">
                                    <img src={testimonial.media.url} alt={testimonial.authorName} className="w-full h-full object-cover" />
                                </div>

                                {/* Header */}
                                <h3 className="text-lg font-bold text-black z-20 relative">{testimonial.authorName}</h3>
                                <p className="text-sm text-[#505050] mb-4">{testimonial.position}</p>

                                {/* Testimonial Text */}
                                <p className="text-sm text-black font-ubuntu relative z-20">{testimonial.content}</p>
                            </div>
                        </div>
                    ))}
                </Slider>
